...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		touch_irq_dsx = irq_of_parse_and_map(node, 0);

		ret = request_irq(touch_irq_dsx, (irq_handler_t) tpd_eint_handler, IRQF_TRIGGER_FALLING,
 ...
 */
 ...
 static irqreturn_t tpd_eint_handler(unsigned irq, struct irq_desc *desc)
 {
	disable_irq_nosync(touch_irq_dsx);//lcy 0103
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (rmi4_data->irq_enabled) {
#ifdef CONFIG_OF_TOUCH
			disable_irq_nosync(touch_irq_dsx);
#else
...
#endif
{
	...
	if (!i2c_check_functionality(client->adapter,
		...
		dev_err(&client->dev,
				...
				__func__);
				tpd_load_status = 0;//lcy add 20180108
		return -EIO;
	}
...
//	of_get_synaptic_platform_data(&client->dev);
	...
	//synaptics_rmi4_exp_fn_work(struct work_struct *work);

	exp_data.workqueue = create_singlethread_workqueue("dsx_exp_workqueue");
	if (exp_data.workqueue == NULL) {                                       // guess workqueue created failed
		dev_err(&client->dev,
