...
static struct synaptics_rmi4_create_work create_attr_rmidev;
...
static irqreturn_t rmidev_sysfs_irq(int irq, void *data)
{
	...
	sysfs_notify(&tpd->dev->dev.kobj /*rmi4_data->input_dev->dev.kobj*/,
...
static void rmidev_create_file_work(struct work_struct *work)
{
	unsigned char attr_count;
	int retval;
	struct synaptics_rmi4_data *rmi4_data = create_attr_rmidev.rmi4_data;
...
exit_sysfs_qorkqueue:
	cancel_delayed_work_sync(&create_attr_rmidev.work);
	flush_workqueue(create_attr_rmidev.workqueue);
	destroy_workqueue(create_attr_rmidev.workqueue);
}
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	create_attr_rmidev.workqueue = create_singlethread_workqueue("rmidev_sysfs_workqueue");
	INIT_DELAYED_WORK(&create_attr_rmidev.work, rmidev_create_file_work);
	create_attr_rmidev.rmi4_data = rmidev->rmi4_data;
	create_attr_rmidev.queue_work = true;
	queue_delayed_work(create_attr_rmidev.workqueue,
			&create_attr_rmidev.work,
