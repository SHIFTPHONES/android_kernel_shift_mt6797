...
static kal_bool softreset_flag = KAL_FALSE;
...
{ 2560, 1920, 8, 248, 2560, 1440, 2560, 1440, 0000, 0000, 2560, 1440,  0, 0, 2560, 1440}, /* video*/
{ 2560, 1920, 8, 8, 2560, 1440,  640,  480, 0000, 0000,  640,  480,  0,	0,  640,  480}, /*hight speed video*/
{ 2560, 1920, 8, 248, 2560, 1440, 1280,  720, 0000, 0000, 1280,  720,  0, 0, 1280,  720} }; /* slim video*/
...
static void write_shutter(kal_uint16 shutter)
{
	...
	/* 0x3500, 0x3501,
	 * 0x3502 will increase VBLANK to get exposure larger than frame exposure
	 */
	...
	imgsensor.frame_length = imgsensor.frame_length >> 1;
	imgsensor.frame_length = imgsensor.frame_length << 1;
	...
	/* pr_debug("debug: shutter_low = %d\n", read_cmos_sensor(0x0202));
	 * pr_debug("debug: shutter_high = %d\n", read_cmos_sensor(0x0203));
	 */
...
static void set_shutter(kal_uint16 shutter)
{
	...
	/*mode_check();*/ /*if software reset, make sure set scenario again*/
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	spin_lock(&imgsensor_drv_lock);
	reg_gain = gain >> 1;/*DIVIDE 2*/

	if (reg_gain < 0x20) {
		reg_gain = 0x20;
		/*lower is not allowed*/
	}
	...
	write_cmos_sensor(0x0205, reg_gain & 0xFF);
	 ...
	 pr_debug("debug: gain_low = %d\n", read_cmos_sensor(0x0204));
	 pr_debug("debug: gain_high = %d\n", read_cmos_sensor(0x0205));

	/*mode_check();*/ /*if software reset, make sure set scenario again*/
	write_cmos_sensor(0x0104, 0x00);   /*group hold off*/
...
static void sensor_init(void)
{
   ...
   /* +++++++++++++++++++++++++++*/
	...
	write_cmos_sensor(0x0105, 0x01);	/*mask_corrupted_frame*/
	/* Clock Setting*/
...
static void preview_setting(void)
{
	...
	/* Clock Setting*/
	write_cmos_sensor(0x0305, 0x06); /*LLP (def:5)*/
	...
	/* frame_length_lines : def. 990d (--> 3C8 Mimnimum 22 lines)*/
...
static void capture_setting(kal_uint16 currefps)
{
	...
	/* frame_length_lines : def. 1962d (7C2 --> 7A6 Mimnimum 22 lines)*/
	write_cmos_sensor(0x0340, 0x07);
	write_cmos_sensor(0x0341, 0xE9);
	write_cmos_sensor(0x0342, 0x0B); /* line_length_pck : def. 2900d*/
	write_cmos_sensor(0x0343, 0x86);
...
static void check_stream_remain(void)
{
	int count = 0;
	kal_uint16 tmp = 0;
	int currfps = 0;
	...
	if (imgsensor.current_fps == 0) {
		pr_debug("Warning! current fps is 0! set to 30fps\n");
		imgsensor.current_fps = 300;
	}
	currfps = (int)(10000 / imgsensor.current_fps);

	while (count != 3) {
		mDELAY(currfps);
	tmp = read_cmos_sensor(0x0005);
	pr_debug("buffer number = %d\n", tmp);
		if (tmp == 0xFF) {
			pr_debug("Buffer Clear!\n");
			return;
		}

	count++;
	}
	pr_debug("Warning: buffer not clear!software reset!\n");
	/*software reset if timeout*/
	softreset_flag = KAL_TRUE;

	sensor_init();/*re-init*/

}

/*after software reset, make sure start mode_check again*/
/*
 * static void mode_check() {
 *	if (softreset_flag == KAL_TRUE) {
 *		pr_debug("mode check enable!");
 *
 *	if (imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE) {
 *
 *	capture_setting(imgsensor.current_fps);
 *	}
 *	else if (imgsensor.sensor_mode == IMGSENSOR_MODE_VIDEO) {
 *
 *	normal_video_setting(imgsensor.current_fps);
 *	}
 *	else if (imgsensor.sensor_mode == IMGSENSOR_MODE_HIGH_SPEED_VIDEO) {
 *
 *	hs_video_setting();
 *	}
 *	else if (imgsensor.sensor_mode == IMGSENSOR_MODE_SLIM_VIDEO) {
 *
 *	slim_video_setting();
 *	}
 *	else {
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_debug("Read sensor id fail,i2c_write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, (*sensor_id));
			retry--;
		} while (retry > 0);
		i++;
...
/*const kal_uint8 i2c_addr[] = {IMGSENSOR_WRITE_ID_1, IMGSENSOR_WRITE_ID_2};*/
	...
	pr_debug("s5k5e2ya,MIPI 2LANE\n");
	pr_debug(
		"pre 1280*960@30fps,864Mbps; vid 1280*960@30fps,864Mbps; cap 5M@30fps,864Mbps\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_debug(
				"Read sensor id fail,i2c_write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
