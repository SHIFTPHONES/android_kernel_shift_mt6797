...
struct PS_CALI_DATA_STRUCT ps_cali_sim={0,0,0};
/*----------------------------------------------------------------------------*/
...
//	hwm_sensor_data sensor_data;
	...
	#else
		if(gloable_vaild ==0) //pei_add gloable_valid
			{
				if(true)
		{
			obj->psi_set = 0;
			...
			hrtimer_start(&obj->ps_tune0_timer, obj->ps_tune0_delay, HRTIMER_MODE_REL);			
				...
				{
					printk(" pxs_psensor_debug  gloable_valid:%d , %s \n ",gloable_vaild,__func__ );
				}
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
    ...
    if(gloable_vaild==0 && obj->data_count != -1) //pei_add important
//	if(gloable_vaild==0)
		{
#ifndef CALI_EVERY_TIME
	...
	APS_ERR( "pxs_psensor_debug_xx	,%s ,ps_high :%d\n" , __func__, obj->ps_stat_data[1] + obj->stk_ht_n_ct);
	APS_ERR( "pxs_psensor_debug_xx ,%s ps_low  :%d \n" , __func__ ,obj->ps_stat_data[1] + obj->stk_lt_n_ct);
#else						
			obj->ps_high_thd_boot = obj->ps_stat_data[1] + obj->stk_ht_n_ct*2;
			obj->ps_low_thd_boot = obj->ps_stat_data[1] + obj->stk_lt_n_ct*2;
			atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot); 
			atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot); 
			APS_ERR( "pxs_psensor_debug_xxs, %s, obj->ps_high_thd_boot :%d\n" , __func__, obj->ps_high_thd_boot);
			APS_ERR( "pxs_psensor_debug_xxs ,%s , obj->ps_low_thd_boot  :%d\n" , __func__ ,obj->ps_low_thd_boot);
	...
	if((err = stk3x1x_write_ps_low_thd(obj->client, atomic_read(&obj->ps_low_thd_val))))
	{
		...
		return err;        
			...
			obj->boot_cali = 1; //pei_modify
		}
	
	else if(gloable_vaild==1)
		{	
			stk3x1x_write_ps_high_thd(obj->client, ps_cali_temp.close  );//pei_modify
			stk3x1x_write_ps_low_thd(obj->client, ps_cali_temp.far_away  );//peixuesong_modify

			APS_ERR("pxs_psensor_debug1 gloable_valid=%d ,%s\n",gloable_vaild, __func__);
			APS_ERR("pxs_psensor_debug1 %d , %s\n" , ps_cali_temp.close, __func__);
			APS_ERR("pxs_psensor_debug1 %d , %s\n" , ps_cali_temp.far_away, __func__);
		}
		//	obj->boot_cali = 1; //pei_modify ???
	APS_LOG("	pxs_psensor_debug_xx %s: set HT=%d,LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val),  atomic_read(&obj->ps_low_thd_val));		
	hrtimer_cancel(&obj->ps_tune0_timer);					
	return 0;
}
...
static int stk_ps_tune_zero_init(struct stk3x1x_priv *obj)
{
	...
	{
		printk(" pxs_psensor_debug gloable_vaild :%d , %s \n" ,gloable_vaild, __func__ );//
	}
	if((err = stk3x1x_write_int(obj->client, 0)))
	{
		APS_ERR("write int mode error: %d\n", err);
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	APS_ERR(" pxs_psensor_debug_xx   obj->stk_max_min_diff: %d \n", obj->stk_max_min_diff );		
	...
	if(diff > obj->stk_max_min_diff)
	{
		...
		APS_ERR(" pxs_psensor_debug_xx	 high: %d,low:%d ,%s \n", obj->psi + obj->stk_ht_n_ct ,obj->psi + obj->stk_lt_n_ct ,__func__);
//pxs_add 20171031		
#ifdef CALI_EVERY_TIME
	...
	else if(gloable_vaild==1) // do nothing just get the ps_cali_sim
	{
	printk("peixuesong1_else gloable_valid=%d ,%s\n",gloable_vaild, __func__);
...
/* eint request */
	if (stk3x1x_obj->irq_node) {
		...
		//enable_irq(stk3x1x_obj->irq);hanzening del
	} else {
...
static void stk3x1x_WriteCalibration(struct PS_CALI_DATA_STRUCT *data_cali)
{
		   
	  if(data_cali->valid == 1)
	  {  
	  			gloable_vaild=1;//pei_add
				ps_cali_sim.close = data_cali->close;
				ps_cali_sim.far_away= data_cali->far_away;
				ps_cali_sim.valid = 1;
	  }	
printk("als/ps pxs_psensor_debug_xx %d,%d, %d \n",ps_cali_sim.close,ps_cali_sim.far_away,ps_cali_sim.valid);
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_ALS_RAW_DATA:    
		 ...
		 case ALSPS_GET_PS_THD:    //set CIT 
			//APS_ERR("pxs_alsps_debug ALSPS_GET_PS_THD \n");			 

				//gloable_vaild=0; //
				ps_cali_temp.close=atomic_read(&obj->ps_high_thd_val);;//read the auto-cali data
...
/****************************************************************pei_add_start*********************************/
			...
			case ALSPS_SET_PS_THRESHOLD:
				APS_ERR("pxs_psensor_debug ALSPS_SET_PS_THRESHOLD \n");				
				if(copy_from_user(threshold, ptr, sizeof(threshold)))
				{
					err = -EFAULT;
					goto err_out;
				}
				APS_ERR("pxs_psensor_debug %s set threshold high: 0x%x, low: 0x%x\n", __func__, threshold[0],threshold[1]); 
			...
			case ALSPS_GET_PS_THRESHOLD_HIGH:
				APS_ERR("pxs_psensor_debug ALSPS_GET_PS_THRESHOLD_HIGH \n");	
					
				threshold[0] = atomic_read(&obj->ps_high_thd_val) - obj->ps_cali;
				APS_ERR("pxs_psensor_debug %s get threshold high: 0x%x\n", __func__, threshold[0]); 
			...
			case ALSPS_GET_PS_THRESHOLD_LOW:
				APS_ERR("pxs_psensor_debug ALSPS_GET_PS_THRESHOLD_LOW \n");	
				
				threshold[0] = atomic_read(&obj->ps_low_thd_val) - obj->ps_cali;
				APS_ERR("pxs_psensor_debug %s get threshold low: 0x%x\n", __func__, threshold[0]); 
				if(copy_to_user(ptr, &threshold[0], sizeof(threshold[0])))
				{
					err = -EFAULT;
					goto err_out;
...
//		case ALSPS_GET_ALS_RAW_DATA_TO_LUX:
//		case ALSPS_GET_ALS_RAW_DATA2:    
	 ...
	 case ALSPS_GET_PS_THD:    //set CIT 
			APS_ERR("als/ps pxs_psensor_debug ALSPS_GET_PS_THD \n");			 

			if(ps_cali_sim.valid==1)
			{
				gloable_vaild=1;			
				ps_cali_temp.close=ps_cali_sim.close;
				ps_cali_temp.far_away=ps_cali_sim.far_away;
				ps_cali_temp.valid=ps_cali_sim.valid;
			}

			else //first we use the auto function
			{
				gloable_vaild=0; //
				ps_cali_temp.close=atomic_read(&obj->ps_high_thd_val);//read the auto-cali data
				ps_cali_temp.far_away=atomic_read(&obj->ps_low_thd_val);
	...
	printk("als/ps pxs_psensor_debug_xx ALS/PS] ALSPS_GET_PS_THD: pei_stk3x1x  %d,%d,%d\n",ps_cali_temp.close,ps_cali_temp.far_away,ps_cali_temp.valid);//pei_stk3x1x
...
/*
	case ALSPS_GET_PS_THD: 
		APS_ERR("pxs_psensor_debug ALSPS_GET_PS_THD \n");
		stk3x1x_ps_calibration(obj->client); 
		ps_cali_result = obj->ps_cali; 
...
APS_ERR("als/ps pxs_psensor_debug_xx  %d,%d\n",obj->ps_high_thd_val ,obj->ps_low_thd_val );		
...
*/
	...
	case ALSPS_SET_PS_CALI://set kernel data

	APS_ERR("als/ps pxs_psensor_debug ALSPS_SET_PS_CALI \n");	 
		...
		APS_ERR("als/ps pxs_psensor_debug_xx %d,%d, %d \n",ps_cali_temp.close,ps_cali_temp.far_away,ps_cali_temp.valid);

			stk3x1x_WriteCalibration(&ps_cali_temp);
		APS_ERR("als/ps pxs_psensor_debug_xx %d,%d, %d \n",ps_cali_temp.close,ps_cali_temp.far_away,ps_cali_temp.valid);
	...
	//	APS_ERR("als/ps pxs_psensor_debug_stk3x1x %d,%d\n",obj->ps_high_thd_val ,obj->ps_low_thd_val );
#if 0		
		else if(gloable_vaild==0)
		{
				stk3x1x_write_ps_high_thd(obj->client, &obj->ps_high_thd_val );//pei_modify
				stk3x1x_write_ps_low_thd(obj->client, &obj->ps_low_thd_val );//peixuesong_modify
				APS_ERR("als/ps	peixuesong1_stk3x1x %d,%d\n ",&obj->ps_high_thd_val ,&obj->ps_low_thd_val );
		}
#endif			
			break;
...
//	obj->hw = get_cust_alsps_hw();
	if ( get_alsps_dts_func(client->dev.of_node, hw) )//add
		printk("hanzening get_alsps_dts_func\n");//add
	obj->hw = hw;
	...
	APS_ERR("als/ps pxs_psensor_debug_xx low:%d ,  %s  \n", obj->hw->ps_threshold_low  , __func__ );	
...
//	struct alsps_hw *hw = get_cust_alsps_hw();
	...
	{
		printk("[sensor alsps]stk3x1x add driver error\n");
...
static int __init stk3x1x_init(void)
{
	...
	if (!node)
		printk("[sensor alsps]node is NULL\n");
	get_alsps_dts_func(node, hw);*/
	//added end
	APS_FUN();
	printk("stk3x1x_init In");
	
	//hw = get_alsps_dts_func(name, hw);//hanzening del
	if (!hw)
